...
- `MODEL`, `MODEL_MAX_TOKENS_PER_CALL` — Anthropic model name and token budget (input + output) per call. The default (10,000 tokens) is intentionally conservative to work on any Anthropic tier. The model supports significantly higher limits, especially on Tier 2+. Increase this value according to your plan to get more detailed and precise answers.
...
If the input exceeds `MODEL_MAX_TOKENS_PER_CALL`, truncatable files (README, file tree, downloaded sources) are automatically shortened. The longest files are leveled down first (e.g. files of 1k, 2k, 3k, 8k chars truncated to a total budget become 1k, 2k, 2k, 2k). Truncation snaps to the nearest newline to avoid cutting mid-line. If file-level truncation is insufficient, the entire prompt is proportionally cut as a fallback.
