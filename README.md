...
- `MODEL`, `MODEL_MAX_TOKENS_PER_CALL` — Anthropic model name and maximum total tokens per call (input + output). Output tokens are capped separately. Each request to `/summarize` makes 2 model calls (first pass + second pass). The default (15,000 tokens) is conservative and works on any Anthropic tier. For claude-sonnet-4-6 you can set this to 200000. Increase this value to get more detailed and precise answers.
